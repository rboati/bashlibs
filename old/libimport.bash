...
__libimport_generate_functions_maps() {
	...
	for fun in "${function_list[@]}"; do
		...
		while read -r line; do
			...
			line=${line//__NS__/$ns}
			#if [[ $line =~ ^[[:space:]]*:[[:space:]]*'require_'([_[:alnum:]]+)[[:space:]]+(.*)\;$ ]]; then
			if [[ $line =~ ^[[:space:]]*'pragma'[[:space:]]+'require_'([_[:alnum:]]+)[[:space:]]+(.*)\;$ ]]; then
...
bash_import() {
	...
	eval "$(exec bash --noprofile --norc -s <<- EOF
			{
		...
		for __libimport_item in "${__libimport_sorted_list[@]}"; do
			...
			if [[ -v __libimport_FUNCTION_CODE_MAP[${__libimport_item}] ]]; then
				...
				if declare -F "$__libimport_item" &> /dev/null; then
					__libimport_log warn "A function '$__libimport_item' already exists, overwriting."
				fi
...
export -f __libimport_generate_functions_maps bash_import