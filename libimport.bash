...
bash_source() {
	...
	if (( found == 0 )); then
		(( LOGLEVEL >= 2 )) && printf '\e[37m%s\e[0m:\e[37m%s\e[0m:%s\n' ERROR bash_source "Sourcing from ${import_type} path: '$source_file' not found! ($__FILE__)" 1>&2
		exit 1
	fi
	...
	if [[ -n ${BASH_IMPORT[$__FILE__]} ]]; then
		...
		for item in "${namespaces[@]}"; do
			if [[ ${ns:-<empty>} == "$item" ]]; then
				(( LOGLEVEL >= 5 )) && printf '\e[37m%s\e[0m:\e[37m%s\e[0m:%s\n' DEBUG bash_source "Sourcing from ${import_type} path: '$source_file' already imported with namespace '${ns:-<empty>}', skipping." 1>&2
				return 2
			fi
		done
		if (( LOGLEVEL >= 5 )); then
			printf '\e[37m%s\e[0m:\e[37m%s\e[0m:%s' DEBUG bash_source "Sourcing from ${import_type} path: '$source_file' already imported with other namespaces (${BASH_IMPORT[$__FILE__]%,}), importing with " 1>&2
			if [[ -z $ns ]]; then
				printf '%s\n' "no namespace." 1>&2
			else
				printf '%s\n' "namespace \"$ns\"." 1>&2
		...
		(( LOGLEVEL >= 5 )) && printf '\e[37m%s\e[0m:\e[37m%s\e[0m:%s\n' DEBUG bash_source "Sourcing from ${import_type} path: '${source_file}'${ns:+ with namespace \"$ns\"}" 1>&2
	fi
